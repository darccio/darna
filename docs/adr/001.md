...
**Important**: Files are sorted **lexicographically** by path before selecting the first independent file. This ensures deterministic, reproducible output across runs.
...
### Dependants Mode (`--dependants` flag)

The `--dependants` flag extends committable mode to include direct dependants of the base file, enabling larger atomic commits when possible.

**Algorithm:**
1. Find the first independent file (same as `--committable`)
2. Identify all files in the changeset that depend on this base file
3. Filter to only "direct dependants" that:
   - Depend on the base file
   - Have NO dependencies on OTHER changeset files (only base file + committed code)
   - Are NOT transitive dependants (dependants of dependants)
4. Return base file + direct dependants (sorted for determinism)

**Example dependency graph:**
```
types.go (independent)
  ↑
  └── service.go (depends ONLY on types.go)
        ↑
        └── main.go (depends on service.go + types.go)
```

**Output:**
- `--committable`: returns `types.go`
- `--committable --dependants`: returns `types.go service.go` (excludes `main.go` as transitive)

**Why exclude transitive dependants:**
- Maintains atomicity: each commit is still independently reviewable
- Prevents cascading dependencies from pulling in the entire changeset
- Enables incremental patchset building: commit base + direct dependants, then their dependants become the new base

**Edge cases:**
- **No dependants**: Returns just the base file
- **Circular dependencies**: Returns empty (no independent base file)
- **Multiple dependants with cross-dependencies**: Only includes dependants that depend ONLY on the base file
  ```
  base.go
    ↑
    ├── dep1.go (depends on base.go) ✓ included
    └── dep2.go (depends on base.go AND dep1.go) ✗ excluded
  ```

**Implementation:** See `findDirectDependants()` in `internal/validator/validator.go`.
...
### 6. Partial Staging Support (Symbol-level)

**Concept**: For modified unstaged files that can't be committed whole, enable partial staging of committable symbols.

**Example scenario:**
```go
// file.go (unstaged)
func FuncA() { /* uses committed code */ }     // Committable.
func FuncB() { /* uses other unstaged code */ } // Not committable.
```

Could stage only `FuncA` along with the base committable file.

**Feasibility investigation:**

The pure Go git implementation [go-git](https://github.com/go-git/go-git) provides:
- ✅ `Worktree.Add(path)` - whole file staging
- ✅ `Worktree.AddGlob(pattern)` - pattern-based staging
- ❌ **No hunk-level or line-level staging support**

**Challenges:**
1. go-git doesn't support partial file staging (hunk/line granularity)
2. Would need to:
   - Generate git patches at symbol boundaries
   - Apply patches using `git apply --cached`
   - Or shell out to `git add -p` with automated responses
3. Symbol boundaries don't align with git hunks - complex mapping needed
4. Requires integration with git porcelain commands, losing pure-Go benefits

**Recommendation**: Document as future enhancement. Consider if/when go-git adds hunk-level staging API, or if git integration becomes priority over pure-Go implementation.

**Sources:**
- [go-git GitHub repository](https://github.com/go-git/go-git)
- [go-git v5 API documentation](https://pkg.go.dev/github.com/go-git/go-git/v5)
